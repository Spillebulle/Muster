...
fn repo_root() -> PathBuf {
    // CARGO_MANIFEST_DIR is crates/muster-desktop.
...
/// `muster_app::update::release::wanted_asset` matches the four kinds the
...
/// `muster_0.0.1_amd64.deb`, rpm adds its release number, and the AppImage is
/// capitalised after the application rather than the crate.
///
/// There is no macOS row. `CLAUDE.md` settles that it is not a target, so the
/// release matrix has four entries where Umber's has five.
const ASSETS: &[&str] = &[
    "muster-setup-{v}-x64.exe",
    "muster-setup-{v}-arm64.exe",
    "muster-{v}-x64.msi",
    "muster-{v}-arm64.msi",
    "muster_{v}_amd64.deb",
    "muster_{v}_arm64.deb",
    "muster-{v}-1.x86_64.rpm",
    "muster-{v}-1.aarch64.rpm",
    "muster-bin-{v}-1-x86_64.pkg.tar.zst",
    "Muster-{v}-x86_64.AppImage",
    "Muster-{v}-aarch64.AppImage",
    "muster-{v}-x86_64.flatpak",
    "muster-{v}-x86_64-pc-windows-msvc.zip",
    "muster-{v}-aarch64-pc-windows-msvc.zip",
    "muster-{v}-x86_64-unknown-linux-gnu.tar.gz",
    "muster-{v}-aarch64-unknown-linux-gnu.tar.gz",
];
...
/// the same every release, and Muster's carry the version — which is worth
...
fn the_readme_links_to_every_file_of_this_release() {
    ...
    // under the words "Muster 0.0.5" is the kind of wrong that gets believed.
    assert!(
        text.contains(&format!("**Muster {version}.**")),
        "README.md's Installing section does not say `**Muster {version}.**`, so \
         it names a different release from the one it links to"
    );
    for asset in ASSETS {
        let name = asset.replace("{v}", version);
        let link =
            format!("https://github.com/Spillebulle/muster/releases/download/v{version}/{name}");
        assert!(
            text.contains(&link),
...
fn the_readme_has_no_download_link_that_is_not_a_file_of_this_release() {
    ...
    const PREFIX: &str = "https://github.com/Spillebulle/muster/releases/download/";
