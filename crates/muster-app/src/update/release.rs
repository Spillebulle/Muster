...
//! every field Muster does not use is dropped on the floor — a release payload
//! carries a hundred keys and Muster needs five.
//!
...
//! version number would mean Muster deciding where to fetch a binary from, which
...
pub const REPOSITORY: &str = "https://github.com/Spillebulle/muster";

/// Where somebody goes to fetch a build by hand — which is the answer for
/// every installation Muster may not update itself.
pub const RELEASES_PAGE: &str = "https://github.com/Spillebulle/muster/releases";
...
pub const API: &str = "https://api.github.com/repos/Spillebulle/muster/releases?per_page=20";
...
impl Asset {
    /// Whether this is an address Muster is willing to fetch from.
    ///
    /// Muster does not sign its releases, so the transport is the whole of the
...
impl Release {
    ...
    /// time, or an installation kind Muster does not update itself. The caller
...
pub enum AssetKind {
    /// `muster-<version>-<arch>.msi`, handed to `msiexec`.
    Msi { suffix: &'static str },
    /// `muster-<version>-<target>.zip`, holding `muster.exe`.
    WindowsZip { suffix: &'static str },
    /// `muster-<version>-<target>.tar.gz`, holding `muster`.
    Tarball { suffix: &'static str },
    /// `Muster-<version>-<arch>.AppImage` — the whole application, one file.
...
/// `muster-${version}-${target}.{zip,tar.gz}` from the Stage step,
/// `muster-${version}-${arch}.msi` from Build MSI, and
/// `Muster-${version}-${arch}.AppImage` from `packaging/linux/build-packages.sh`.
...
pub fn wanted_asset(kind: &InstallKind, os: Os, arch: Arch) -> Option<AssetKind> {
    match kind {
        ...
        // A package manager's, or somewhere Muster cannot place itself. There is
...
/// What the API returns, as far as Muster reads it.
#[derive(Deserialize)]
struct RawRelease {
    tag_name: String,
    #[serde(default)]
...
///
/// Returns `None` when the reply holds no release Muster recognises, which is
...
mod tests {
    ...
    /// A reply shaped like GitHub's, cut down to the fields Muster reads plus a
    ...
    const FIXTURE: &str = r#"[
      {
        ...
        "draft": false,
        "prerelease": false,
        "html_url": "https://github.com/Spillebulle/muster/releases/tag/v0.0.9",
        "body": "Added\n- Nothing yet.\n",
        "author": { "login": "Spillebulle", "id": 1 },
        "assets": [
          {
            "name": "muster-0.0.9-x64.msi",
            "size": 41,
            "content_type": "application/octet-stream",
            "browser_download_url": "https://github.com/Spillebulle/muster/releases/download/v0.0.9/muster-0.0.9-x64.msi"
          }
        ]
      },
      {
        ...
        "draft": false,
        "prerelease": false,
        "html_url": "https://github.com/Spillebulle/muster/releases/tag/v0.0.10",
        "body": "The tenth.\n",
        "assets": [
          { "name": "muster-0.0.10-x64.msi", "size": 10, "browser_download_url": "https://github.com/x/muster-0.0.10-x64.msi" },
          { "name": "muster-0.0.10-arm64.msi", "size": 11, "browser_download_url": "https://github.com/x/muster-0.0.10-arm64.msi" },
          { "name": "muster-0.0.10-x86_64-pc-windows-msvc.zip", "size": 12, "browser_download_url": "https://github.com/x/muster-0.0.10-x86_64-pc-windows-msvc.zip" },
          { "name": "muster-0.0.10-aarch64-pc-windows-msvc.zip", "size": 13, "browser_download_url": "https://github.com/x/muster-0.0.10-aarch64-pc-windows-msvc.zip" },
          { "name": "muster-0.0.10-x86_64-unknown-linux-gnu.tar.gz", "size": 15, "browser_download_url": "https://github.com/x/muster-0.0.10-x86_64-unknown-linux-gnu.tar.gz" },
          { "name": "muster-0.0.10-aarch64-unknown-linux-gnu.tar.gz", "size": 16, "browser_download_url": "https://github.com/x/muster-0.0.10-aarch64-unknown-linux-gnu.tar.gz" },
          { "name": "Muster-0.0.10-x86_64.AppImage", "size": 17, "browser_download_url": "https://github.com/x/Muster-0.0.10-x86_64.AppImage" },
          { "name": "Muster-0.0.10-aarch64.AppImage", "size": 18, "browser_download_url": "https://github.com/x/Muster-0.0.10-aarch64.AppImage" },
          { "name": "muster_0.0.10_amd64.deb", "size": 19, "browser_download_url": "https://github.com/x/muster_0.0.10_amd64.deb" },
          { "name": "muster-0.0.10-1.x86_64.rpm", "size": 20, "browser_download_url": "https://github.com/x/muster-0.0.10-1.x86_64.rpm" },
          { "name": "muster-0.0.10-x86_64.flatpak", "size": 21, "browser_download_url": "https://github.com/x/muster-0.0.10-x86_64.flatpak" },
          { "name": "muster-0.0.10-1-x86_64.pkg.tar.zst", "size": 22, "browser_download_url": "https://github.com/x/muster-0.0.10-1-x86_64.pkg.tar.zst" }
        ]
      },
      {
        "tag_name": "v0.1.0",
        "draft": true,
        "prerelease": false,
        "html_url": "https://github.com/Spillebulle/muster/releases/tag/v0.1.0",
        "assets": []
      },
      {
        ...
        "draft": false,
        "prerelease": true,
        "html_url": "https://github.com/Spillebulle/muster/releases/tag/v0.2.0-rc.1",
        "assets": []
      },
      {
        "tag_name": "nightly",
        "draft": false,
        "prerelease": false,
        "html_url": "https://github.com/Spillebulle/muster/releases/tag/nightly",
        "assets": []
      },
      {
        "tag_name": "v9.9.9",
        "draft": false,
        "prerelease": true,
        "html_url": "https://github.com/Spillebulle/muster/releases/tag/v9.9.9",
        "assets": []
      }
    ]"#;
    ...
    fn the_newest_release_is_found_by_version_not_by_list_order() {
        ...
        assert_eq!(
            release.page,
            "https://github.com/Spillebulle/muster/releases/tag/v0.0.10",
        );
    }
    ...
    #[test]
    fn a_repository_with_no_release_muster_recognises_yields_nothing() {
    ...
    fn each_platform_and_architecture_picks_its_own_asset() {
        let release = newest_of(FIXTURE);
        let appimage = InstallKind::AppImage(PathBuf::from("/home/a/Muster.AppImage"));
        let cases: [(&InstallKind, Os, Arch, &str); 7] = [
            (
                &InstallKind::Msi,
                Os::Windows,
                Arch::X86_64,
                "muster-0.0.10-x64.msi",
            ),
            (
                &InstallKind::Msi,
                Os::Windows,
                Arch::Aarch64,
                "muster-0.0.10-arm64.msi",
            ),
            (
                &InstallKind::Portable,
                Os::Windows,
                Arch::X86_64,
                "muster-0.0.10-x86_64-pc-windows-msvc.zip",
            ),
            (
                &InstallKind::Portable,
                Os::Windows,
                Arch::Aarch64,
                "muster-0.0.10-aarch64-pc-windows-msvc.zip",
            ),
            (
                &InstallKind::Portable,
                Os::Linux,
                Arch::X86_64,
                "muster-0.0.10-x86_64-unknown-linux-gnu.tar.gz",
            ),
            (
                &appimage,
                Os::Linux,
                Arch::X86_64,
                "Muster-0.0.10-x86_64.AppImage",
            ),
            (
                &appimage,
                Os::Linux,
                Arch::Aarch64,
                "Muster-0.0.10-aarch64.AppImage",
            ),
        ];
    ...
    fn the_linux_arm_tarball_is_not_confused_with_the_windows_one() {
        ...
        assert_eq!(asset.name, "muster-0.0.10-aarch64-unknown-linux-gnu.tar.gz");
    }
    ...
    fn an_asset_that_is_not_on_https_is_treated_as_absent() {
        // Muster does not sign its releases, so the transport is the whole of
        // the guarantee. An asset offered over plain http throws that away.
        let json = r#"[{
          "tag_name": "v1.0.0", "draft": false, "prerelease": false,
          "html_url": "https://github.com/Spillebulle/muster/releases/tag/v1.0.0",
          "assets": [
            { "name": "muster-1.0.0-x64.msi", "size": 1,
              "browser_download_url": "http://github.com/x/muster-1.0.0-x64.msi" }
          ]
        }]"#;
    ...
    fn a_release_with_no_asset_for_this_machine_is_not_an_error() {
        ...
        let json = r#"[{
          ...
          "html_url": "https://github.com/Spillebulle/muster/releases/tag/v1.0.0",
          "assets": [
            { "name": "muster-1.0.0-x86_64.flatpak", "size": 1,
              "browser_download_url": "https://github.com/x/muster-1.0.0-x86_64.flatpak" }
          ]
        }]"#;
        let release = newest_of(json);
