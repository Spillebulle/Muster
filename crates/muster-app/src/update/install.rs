//! How this copy of Muster was installed — and therefore whether it may replace
//! itself.
//!
//! Muster ships in nine shapes (see `packaging/` and `.github/workflows/release.yml`),
//! and they divide into two groups that must be treated completely differently:
//!
//! * **The ones Muster owns.** A portable zip or tarball unpacked wherever the
...
//!   to fail.
//!
//! The MSI is the one managed case Muster can still update, because Windows
//! provides the mechanism: hand `msiexec` a newer MSI and it does the upgrade,
//! elevating on its own and keeping its own record straight. What Muster must
...
//! exist" predicate. That is what lets the Linux answers be tested on a
//! Windows machine, which is the only way they get tested at all.
//!
//! There is no macOS here. `CLAUDE.md` settles that it is not a target, so the
//! [`Os`] enum has two variants rather than three and a build for it would fail
//! to compile in [`Os::CURRENT`] rather than quietly reporting the wrong
//! answer.

use std::path::{Path, PathBuf};

/// The platforms Muster publishes for.
...
impl Arch {
    /// The architecture this build runs on, or `None` on one Muster does not
...
impl Manager {
    ...
    pub fn command(self) -> Option<&'static str> {
        match self {
            Self::Flatpak => Some("flatpak update io.github.spillebulle.muster"),
            Self::Dpkg => Some("sudo apt install --only-upgrade muster"),
            Self::Pacman => Some("sudo pacman -Syu muster"),
...
pub enum InstallKind {
    /// An archive the user unpacked. The binary sits in a directory Muster owns
    ...
    Msi,
    /// Owned by a package manager. Muster says so and stays out of the way.
    Managed(Manager),
    /// Muster could not work out where it is — `current_exe` failed, or the
...
impl InstallKind {
    /// Whether Muster may update this installation itself.
    ...
    /// The sentence shown where an update cannot be applied from inside Muster.
    pub fn cannot_update(&self) -> Option<String> {
        match self {
            ...
            Self::Managed(m) => Some(match m.command() {
                Some(command) => format!(
                    "This copy was installed by {}, which keeps its own record of \
                     every file it owns. Muster will not write over that: the next \
            ...
            Self::Unknown => Some(
                "Muster cannot tell how this copy was installed, so it will not \
...
pub fn detect(probe: &Probe<'_>) -> InstallKind {
    ...
    // sandbox the executable is at `/app/bin/muster`, which is a system path
    ...
    // No architecture Muster publishes for means no asset to offer, whatever
    ...
    match probe.os {
        Os::Windows => {
            ...
            // the reverse mistake would have Muster writing into a directory
            // the installer owns.
...
///
/// dpkg first and by name: `/var/lib/dpkg/info/muster.list` is dpkg's own record
...
fn linux_manager(probe: &Probe<'_>) -> Manager {
    ...
    if exists("/var/lib/dpkg/info/muster.list") {
        Manager::Dpkg
        ...
        // dpkg is here but has no record of muster — the tarball unpacked into
...
/// a `Path` built from `C:\Program Files\Muster` is three components on Windows
...
fn parent_dir(os: Os, exe: &Path) -> Option<PathBuf> {
    match os {
        ...
        Os::Linux => exe.parent().map(Path::to_path_buf),
    }
}
...
mod tests {
    ...
    fn a_flatpak_is_recognised_before_anything_else() {
        // Inside the sandbox the binary is at /app/bin/muster, and a Debian
        ...
        let p = probe(
            ...
            Some("/app/bin/muster"),
            &env,
            &["/.flatpak-info", "/var/lib/dpkg/info/muster.list"],
        );
    ...
    fn the_flatpak_id_alone_is_enough() {
        let mut env = no_env();
        env.insert("FLATPAK_ID", "io.github.spillebulle.muster");
        let p = probe(Os::Linux, Some("/app/bin/muster"), &env, &[]);
        assert_eq!(detect(&p), InstallKind::Managed(Manager::Flatpak));
    }
    ...
    fn an_appimage_is_the_file_the_variable_names() {
        let mut env = no_env();
        env.insert("APPIMAGE", "/home/a/Apps/Muster-0.0.1-x86_64.AppImage");
        let p = probe(
            Os::Linux,
            // The payload runs from a temporary mount, which is exactly why the
            // image's own path has to come from the environment.
            Some("/tmp/.mount_Muster1a/usr/bin/muster"),
            &env,
            &["/home/a/Apps/Muster-0.0.1-x86_64.AppImage"],
        );
        assert_eq!(
            detect(&p),
            InstallKind::AppImage(PathBuf::from("/home/a/Apps/Muster-0.0.1-x86_64.AppImage")),
        );
    }
    ...
    fn an_appimage_whose_file_has_gone_is_not_updated() {
        ...
        let p = probe(Os::Linux, Some("/tmp/.mount_x/usr/bin/muster"), &env, &[]);
        assert_ne!(
            detect(&p),
    ...
    fn a_windows_install_under_program_files_is_the_msi() {
        ...
        for exe in [
            "C:\\Program Files\\Muster\\muster.exe",
    ...
    fn a_windows_zip_unpacked_anywhere_else_is_portable() {
        ...
        env.insert("ProgramFiles", "C:\\Program Files");
        for exe in [
            "C:\\Users\\a\\Downloads\\muster\\muster.exe",
            "D:\\portable\\muster\\muster.exe",
            // A textual prefix test would call this one an MSI install.
            "C:\\Program Files Custom\\muster\\muster.exe",
        ] {
            let p = probe(Os::Windows, Some(exe), &env, &[]);
    ...
    fn a_debian_package_is_named_by_dpkgs_own_record() {
        ...
        let p = probe(
            ...
            Some("/usr/bin/muster"),
            &env,
            &["/var/lib/dpkg/info/muster.list", "/var/lib/dpkg/status"],
        );
    ...
    fn an_rpm_distribution_is_recognised_by_either_database_location() {
        ...
        for present in [
            ...
            let p = probe(Os::Linux, Some("/usr/bin/muster"), &env, present);
            assert_eq!(
                detect(&p),
    ...
    fn a_system_install_with_no_database_is_still_not_ours_to_replace() {
        let env = no_env();
        let p = probe(Os::Linux, Some("/opt/muster/muster"), &env, &[]);
    ...
    #[test]
    fn only_the_three_shapes_muster_owns_may_replace_themselves() {
    ...
    fn a_managed_install_names_the_manager_it_belongs_to() {
        ...
        assert!(
            flatpak.contains("flatpak update io.github.spillebulle.muster"),
            "{flatpak}",
        );
