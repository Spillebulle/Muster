...
//!
//! Three shapes, one per installation Muster owns (see [`super::install`]):
//!
...
//!   `$APPIMAGE` names.
//! * **MSI** — write the package down and start Muster's own updater over it.
//!   Muster does not touch Program Files itself: the installer owns those files
...
//!   left the artist to start Muster again themselves; `super::installer` has
...
//! binary is moved aside to `muster.exe.old`, the new one takes its name, and
...
//! Muster rather than none at all.
//!
...
/// gigabyte of `muster` is a decompression bomb or a mistake, and either way is
...
pub enum Applied {
    /// The new build is in place and runs the next time Muster is started.
    Restart,
    /// An installer is now running and needs Muster to close.
    Installer,
}
...
/// MSI is not installed by Muster at all — it is handed over.
pub fn apply(
    kind: &InstallKind,
    asset_name: &str,
    bytes: &[u8],
    ...
    match kind {
        ...
        InstallKind::Portable => {
            ...
            let binary = if asset_name.ends_with(".zip") {
                binary_from_zip(bytes, "muster.exe")?
            } else {
                binary_from_tar_gz(bytes, "muster")?
            };
            report(Stage::Installing);
            swap_in(&exe, &binary)?;
            Ok(Applied::Restart)
        ...
        InstallKind::Managed(_) | InstallKind::Unknown => Err(
            "This installation is not Muster's to replace. Take the new build from \
             the releases page."
                .to_string(),
        ),
    }
...
fn swap_in(path: &Path, bytes: &[u8]) -> Result<(), String> {
    ...
    // inode open, so nothing is disturbed until Muster is next started.
...
fn write_executable(path: &Path, bytes: &[u8]) -> Result<(), String> {
    ...
    #[cfg(unix)]
    {
        ...
        std::fs::set_permissions(path, std::fs::Permissions::from_mode(0o755))
            .map_err(|e| format!("Muster could not make {} executable: {e}", path.display()))?;
    }
    Ok(())
}
...
/// `muster.exe` gives `muster.new`, which is a *different program's* name on a
...
/// go. Failure is ignored: a stale `muster.exe.old` is untidy, not broken, and
...
/// Start the build that is now at Muster's own path, so the caller can exit into
/// it.
///
...
/// old one running, not leave the user with no Muster at all. `app.rs` exits only
/// on `Ok`.
///
/// On Windows the displaced `muster.exe.old` is still locked by *this* process
...
pub fn relaunch() -> Result<(), String> {
    let exe = std::env::current_exe()
        .map_err(|e| format!("Muster could not find its own program file: {e}"))?;
    // No arguments carried over. Muster's are a document to open, and reopening
    ...
    std::process::Command::new(&exe)
        ...
        .map_err(|e| {
            format!(
                "Muster could not start the new version: {e}\n\n\
                 It is installed at {}, and runs the next time you start Muster.",
                exe.display(),
            )
        })
}
...
/// hostile or corrupt archive cannot make Muster allocate until it dies.
fn read_capped(reader: &mut impl Read, declared: u64) -> Result<Vec<u8>, String> {
    if declared > MAX_BINARY {
        return Err("The download claims to hold a file far larger than Muster.".to_string());
    }
    ...
    if out.len() as u64 > MAX_BINARY {
        return Err("The download unpacks to far more than Muster's own size.".to_string());
    }
    Ok(out)
}

/// Write the package somewhere the installer can reach it, and start Muster's
/// own updater over it.
///
...
/// asked Muster for an update watched a Windows installer they had not asked for
/// and then had to start Muster again themselves. `installwin::spawn` puts a
/// copy of this executable in the same directory and starts it with
/// `--install-update`; that process draws Muster's own window, waits for this
...
/// Program Files. Muster still does not touch those files itself.
fn hand_to_msiexec(asset_name: &str, bytes: &[u8], version: &str) -> Result<(), String> {
    ...
    let name = if name.ends_with(".msi") {
        ...
        "muster-update.msi".to_string()
    };

    let path = std::env::temp_dir().join(name);
    std::fs::write(&path, bytes).map_err(|e| {
        format!(
            "Muster could not write the installer to {}: {e}",
            path.display()
        )
    })?;
...
mod tests {
    ...
    impl Scratch {
        fn new(name: &str) -> Self {
            ...
            let dir = std::env::temp_dir()
                .join(format!("muster-update-test-{name}-{}", std::process::id()));
    ...
    fn a_suffix_is_added_to_the_whole_name_not_swapped_for_the_extension() {
        // `with_extension` would give `muster.new`, losing the `.exe` Windows
        // needs to run the file and colliding with any other `muster.*` beside
        // it.
        assert_eq!(
            suffixed(Path::new("/a/muster.exe"), ".new"),
            PathBuf::from("/a/muster.exe.new"),
        );
        assert_eq!(
            suffixed(Path::new("/a/muster"), ".old"),
            PathBuf::from("/a/muster.old"),
        );
    }
    ...
    fn a_swap_replaces_the_file_and_leaves_nothing_half_written() {
        ...
        let target = scratch.path("muster.exe");
    ...
    fn a_swap_over_a_leftover_from_a_previous_update_still_works() {
        // Windows cannot rename onto an existing name, so an `muster.exe.old`
    ...
    fn a_zip_gives_up_its_binary_and_says_so_when_it_has_none() {
        let mut buf = Vec::new();
        {
            ...
            zip.start_file("muster-1.0.0-x86_64-pc-windows-msvc/README.md", stored)
                .expect("start README");
            std::io::Write::write_all(&mut zip, b"not the binary").expect("write README");
            zip.start_file("muster-1.0.0-x86_64-pc-windows-msvc/muster.exe", stored)
        ...
        assert_eq!(
            binary_from_zip(&buf, "muster.exe").expect("the binary is found"),
            b"MZ the new build",
        );
        // A release that shipped the wrong archive must be a refusal, not an
        // empty file written over the running one.
        assert!(binary_from_zip(&buf, "muster").is_err());
        assert!(binary_from_zip(b"not a zip at all", "muster.exe").is_err());
    }
    ...
    fn a_tarball_gives_up_its_binary_and_says_so_when_it_has_none() {
        let mut gz = flate2::write::GzEncoder::new(Vec::new(), flate2::Compression::fast());
        {
            ...
            header.set_cksum();
            tar.append_data(&mut header, "muster-1.0.0/LICENSE", &b"GPL3"[..])
            ...
            header.set_cksum();
            tar.append_data(&mut header, "muster-1.0.0/muster", &payload[..])
        ...
        assert_eq!(
            binary_from_tar_gz(&bytes, "muster").expect("the binary is found"),
            b"\x7fELF the new build",
        );
        assert!(binary_from_tar_gz(&bytes, "muster.exe").is_err());
        assert!(binary_from_tar_gz(b"not a tarball", "muster").is_err());
    }
    ...
    fn a_managed_installation_is_refused_even_with_bytes_in_hand() {
        ...
        for kind in [
            ...
            assert!(apply(&kind, "muster-1.0.0-x64.msi", b"anything", "1.0.0", &|_| {}).is_err());
        }
    }
