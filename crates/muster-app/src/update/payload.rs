...
//!
//! `muster-setup.exe` is Muster's own binary with an MSI concatenated onto it and
...
//! same window an update uses — so somebody installing Muster for the first time
//! sees Muster's interface rather than Windows Installer's.
//!
//! ```text
//! +-------------------+
//! |  muster.exe (PE)   |
...
//! after the last section are simply ignored: `muster-setup.exe` runs as Muster
...
//! about where the package came from. Muster does not sign its releases — see
...
///
/// Muster's MSI is about ten megabytes. The bound is not about the real file: it
...
///
/// `None` for a plain executable, which is the ordinary case — `muster.exe`
...
/// carries a package installs it and the file that does not runs as Muster,
...
/// installer, and refusing to start Muster over it would be the wrong way round.
...
mod tests {
    ...
    fn scratch(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("muster-payload-{}", std::process::id()));
    ...
    /// whole of the dispatch, so `muster-setup.exe` started as the application
    ...
    fn a_file_carrying_a_package_is_recognised_by_its_last_bytes_alone() {
        ...
        // And the ordinary case, which is every launch of Muster itself.
        let plain = scratch("muster.exe");
    ...
    /// The ordinary case: `muster.exe` has no payload and must not be read as
    /// though it had one.
    #[test]
