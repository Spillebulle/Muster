//! Checking GitHub for a newer Muster, and installing one where that is
//! legitimate.
//!
...
//!    reported. Muster does **not** sign its releases, so that is the whole of
...
/// windowing layer — the same reason `muster-core` knows nothing about wgpu.
...
pub enum Status {
    ...
    UpToDate,
    /// A newer release exists. Whether Muster can install it is a separate
...
/// here, while the Windows installer needs Muster *gone* and will start the new
/// version itself.
...
pub struct Updates {
    ...
    /// application: Muster is not a service, it is a tool somebody opens when
...
impl Default for Updates {
    fn default() -> Self {
        Self {
            ...
            // cannot change while Muster is running, and asking the file system
...
impl Updates {
    ...
    /// Why Muster does not ask GitHub at all on this installation, if it does not.
    ///
    /// The Flatpak is the one case, for two reasons that agree. Its sandbox is
    /// granted no network — `packaging/linux/io.github.spillebulle.muster.yml`
    ...
    /// every graphical software centre, keep the bundle current without Muster's
    /// help. Opening the sandbox so Muster can duplicate that is the wrong
    /// trade.
    pub fn check_unavailable(&self) -> Option<&'static str> {
        matches!(self.kind, InstallKind::Managed(install::Manager::Flatpak)).then_some(
            "Flatpak keeps this copy up to date. Run `flatpak update \
             io.github.spillebulle.muster`, or leave it to your software centre. \
             Muster will not check for itself, because it could not install what \
             it found: this copy is the sandbox's to replace.",
        )
    }
    ...
    /// second flag would be two things that can disagree about whether Muster
    /// checks.
    ...
    ///
    /// Does nothing unless there is one *and* this installation is Muster's to
    ...
    pub fn install_offered(&mut self) {
        ...
        self.spawn("muster-update-fetch", move |reporter| {
    ...
    /// Stop the restart countdown, leaving Muster running.
    ...
    /// Ask the event loop to close, and — for a copy Muster replaced itself —
    ...
    /// The restart could not be started, so Muster is still running and has to
    /// say why.
    ...
    fn spawn(&mut self, name: &str, job: impl FnOnce(&Reporter) + Send + 'static) {
        ...
        match spawned {
            ...
            Err(e) => {
                ...
                let message = format!("Muster could not start the update check: {e}");
                match self.flow.as_mut() {
                    Some(flow) if flow.holds_work() => flow.failed(message),
                    _ => self.status = Status::Failed(message),
                }
...
///
/// Nobody working on Muster can perform a real update against a real release —
...
pub fn demo_release() -> Release {
    ...
    version.patch += 1;
    // Every asset the release workflow publishes for a machine Muster can update
    ...
    let asset = |name: &str| Asset {
        name: format!("muster-{version}{name}"),
        size: 31 * 1024 * 1024,
        browser_download_url: format!("https://example.invalid/muster-{version}{name}"),
    };
    Release {
        version,
        ...
        assets: vec![
            ...
            Asset {
                name: format!("Muster-{version}-x86_64.AppImage"),
                size: 31 * 1024 * 1024,
                browser_download_url: format!(
                    "https://example.invalid/Muster-{version}-x86_64.AppImage"
                ),
            },
        ],
    }
...
mod tests {
    ...
    fn release() -> Release {
        Release {
            ...
            notes: String::new(),
            assets: vec![Asset {
                name: "muster-9.9.9-x86_64-unknown-linux-gnu.tar.gz".into(),
                size: 1,
