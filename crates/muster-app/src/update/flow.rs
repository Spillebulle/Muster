...
/// stage is invented to make the bar move: the one place Muster cannot report
...
pub enum Stage {
    ...
    ///
    /// Deliberately **not** called verifying. Muster does not sign its releases;
...
impl Countdown {
    ...
    /// Stop it. The dialog stays up and Muster carries on running.
...
/// switching the check off is unavailable. The two that vary are whether Muster
/// may carry the update out itself, and, where it may not, why.
#[derive(Clone, Debug, PartialEq, Eq, Default)]
pub struct Actions {
    /// Draw "Update now". Only where this installation is Muster's to replace
    ...
    pub open_page: bool,
    /// Why Muster will not do it itself: a package manager owns this copy, or
    /// Muster cannot tell where it is.
    pub obstacle: Option<String>,
    /// The installation is one Muster could replace, but the release published
...
impl Flow {
    ...
    /// Stop the countdown, leaving Muster running. Returns false where there is
...
mod tests {
    ...
    fn release() -> Release {
        Release {
            ...
            tag: "v9.9.9".into(),
            page: "https://github.com/Spillebulle/muster/releases/tag/v9.9.9".into(),
            notes: "Added\n- A thing.\n".into(),
            assets: vec![Asset {
                name: "muster-9.9.9-x64.msi".into(),
                size: 40,
                browser_download_url: "https://github.com/x/muster-9.9.9-x64.msi".into(),
            }],
        }
    }
    ...
    fn cancelling_the_countdown_stops_it_for_good() {
        ...
        // cancelled, and Muster carries on running with the new build waiting
        // for the next start.
    ...
    fn no_stage_calls_anything_verified() {
        // Muster does not sign its releases. Every guarantee is about the
    ...
    #[test]
    fn only_an_installation_muster_owns_is_offered_update_now() {
        for kind in [
            InstallKind::Portable,
            InstallKind::Msi,
            InstallKind::AppImage(PathBuf::from("/home/a/Muster.AppImage")),
        ] {
    ...
    fn a_release_with_no_build_for_this_machine_says_so_rather_than_nothing() {
        // Muster owns this copy and would happily replace it; the release simply
