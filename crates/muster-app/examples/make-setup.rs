//! Build `muster-setup.exe` from a binary and a package.
//!
//! ```sh
//! cargo run -p muster-app --example make-setup -- \
//!     target/release/muster.exe muster-0.0.8-x64.msi muster-setup-0.0.8-x64.exe
//! ```
//!
//! The setup executable is Muster's own binary with the MSI concatenated onto it
//! and a footer saying how long the MSI is — `muster_app::update::payload` has
//! the format and the reasoning. Run with `--install` it lifts the package back
//! out and installs it through Muster's own window.
//!
...
//! out of the file's own stem, so `muster-setup-0.0.8-x64.exe` is what puts
//! "Install Muster 0.0.8" at the top of the window.

use muster_app::update::payload;
...
fn main() -> std::process::ExitCode {
    ...
    // an installer rather than Muster. A setup binary that fails it is one that
    ...
    if !payload::carried_by(std::path::Path::new(out)) {
        eprintln!(
            "{out} was written but is not recognised as an installer; it would \
             open Muster instead of installing it"
        );
        return std::process::ExitCode::FAILURE;
    }
    println!(
